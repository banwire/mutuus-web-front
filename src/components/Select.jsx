...
const SelectField = ({
    ...
    return (
        <Grid container item xs={12} sm={12} md={12} lg={12} xl={12} margin={3} style={{ textAlign: 'left' }} flexDirection={'column'} justifyContent={'center'}>
            <Typography variant="body2" color={'black'} style={{ fontSize: '1.5rem', fontFamily: 'Montserrat', fontWeight: 'bolder', ...styleLabel }}>{label}</Typography>
            <Select options={data} isSearchable={isSearch} placeholder={placeholder} onChange={(value) => helpers.setValue(value.value)} styles={{
                control: (baseStyles, state) => ({
                    ...baseStyles,
                    borderColor: state.isFocused ? 'grey' : '#AED43A',
                    color: '#AED43A',
            ...
            {errors[name] &&
                <Typography variant="body2" color={'red'} style={{ fontSize: '1.5rem', fontFamily: 'Montserrat', ...styleError }}>
                    {
                        errors[name]
                    }
                </Typography>
